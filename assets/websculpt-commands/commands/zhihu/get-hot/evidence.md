...
The browser exploration guide was read before using browser automation. A browser session was already attached through `playwright-cli` and confirmed ready for automation.
...
- Direct unauthenticated API access to Zhihu's hot-list API endpoint returned `AuthenticationError`, so this command should rely on browser rendering instead of the API.
